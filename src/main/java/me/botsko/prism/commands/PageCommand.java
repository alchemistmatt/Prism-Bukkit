...
public class PageCommand implements SubHandler {
    ...
    public void handle(CallInfo call) {
        ...
        if (results.getActionResults().isEmpty()) {
            call.getSender().sendMessage(Prism.messenger
                  .playerError("Nothing found." + ChatColor.GRAY + " Either you're missing something, or we are."));
            return;
        }

        call.getSender().sendMessage(Prism.messenger.playerHeaderMsg(
              "Showing " + results.getTotalResults() + " results. Page " + page + " of " + results.getTotalPages()));
        final List<Handler> paginated = results.getPaginatedActionResults();
        if (paginated == null || paginated.size() == 0) {
            call.getSender().sendMessage(
                  Prism.messenger.playerError("Pagination can't find anything. Do you have the right page number?"));
            return;
        }
